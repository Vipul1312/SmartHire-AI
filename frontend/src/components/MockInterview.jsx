...
export default function MockInterview({ resumeText = "" }) {
  ...
  const timerColor = timeLeft > 30 ? "#00ff88" : timeLeft > 10 ? "#ffd700" : "#ff6b6b";
  ...
  const submitAnswer = async (ans) => {
    ...
    setLoading(true);
    try {
      const res = await evaluateAnswer(questions[currentQ], ans, jobRole);
  ...
  const goToNext = () => {
    if (currentQ < questions.length - 1) {
      ...
      setCurrentQ(next); setAnswer(""); setFeedback("");
      setPhase("interview");
      startTimer(difficulty);
    } else {
      setPhase("summary");
  ...
  const Timer = () => (
    <div style={{ display:"flex", alignItems:"center", gap:"16px", padding:"12px 16px", background:"rgba(0,0,0,0.2)", borderRadius:"12px", marginBottom:"16px", border:`1px solid ${timerColor}30` }}>
      <div style={{ position:"relative", width:"56px", height:"56px", flexShrink:0 }}>
        <svg width="56" height="56" style={{ transform:"rotate(-90deg)" }}>
          <circle cx="28" cy="28" r="24" fill="none" stroke="var(--border)" strokeWidth="4"/>
          <circle cx="28" cy="28" r="24" fill="none" stroke={timerColor}
        ...
        <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"13px", fontWeight:800, color:timerColor }}>
          {timeLeft}s
        </div>
      </div>
      <div>
        <div style={{ fontSize:"11px", color:"var(--text-dim)", textTransform:"uppercase", letterSpacing:"0.05em" }}>Time Remaining</div>
        <div style={{ fontSize:"14px", fontWeight:700, color:timerColor, marginTop:"2px" }}>
          {timeLeft > 60 ? "Take your time ✓" : timeLeft > 20 ? "⚡ Hurry up!" : "🔴 Time almost up!"}
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ marginBottom:"16px" }}>
      ...
      <div style={{ display:"flex", gap:"6px", marginTop:"8px", justifyContent:"center" }}>
        {questions.map((_,i) => {
          ...
          const color = i===currentQ ? "#00ff88" : sc?.skipped ? "#ffd700" : sc ? "#00ff88" : "var(--border)";
    ...
    <div style={{ maxWidth:"800px", margin:"0 auto", padding:"32px 16px" }}>
      ...
      {phase === "interview" && questions.length > 0 && (
        <div>
          ...
          <div style={{ ...s.card, borderColor:"rgba(0,255,136,0.3)" }}>
            <div style={{ fontSize:"12px", color:"var(--text-dim)", marginBottom:"8px", textTransform:"uppercase", letterSpacing:"0.05em" }}>Question</div>
          ...
          <div style={s.card}>
            ...
            <div style={{ display:"flex", gap:"10px", marginTop:"16px", flexWrap:"wrap" }}>
              ...
              <button style={s.btnGhost} onClick={skipQuestion}>⏭ Skip</button>
              <button style={s.btnDanger} onClick={()=>{stopTimer();setPhase("summary");}}>🏁 Finish</button>
            </div>
          </div>
        </div>
      )}
      ...
      {phase === "summary" && (
        <div>
          ...
          <button style={{ ...s.btn, width:"100%", padding:"13px" }} onClick={reset}>🔄 Start New Interview</button>
        </div>
      )}
    </div>
  );
